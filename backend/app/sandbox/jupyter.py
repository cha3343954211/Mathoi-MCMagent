        ...
        for attempt in range(1, _KERNEL_START_RETRIES + 1):
            try:
                ...
                await self._silent_exec(
                    ...
                    "_cjk_font = next((f for f in _CJK_PREFER if f in _installed), None)\n"
                    "if _cjk_font is None:\n"
                    "    # 最后兜底：扫描任意含 CJK/Hei/Kai/Song/Gothic 关键词的字体\n"
                    "    _cjk_kw = ('CJK','Hei','Kai','Song','Ming','Gothic',\n"
                    "               'Yahei','SimSun','Noto','Source Han','Wenquan')\n"
                    "    _fallback = [f.name for f in _fm.fontManager.ttflist\n"
                    "                 if any(k.lower() in f.name.lower() for k in _cjk_kw)]\n"
                    "    _cjk_font = _fallback[0] if _fallback else None\n"
                    "_sans = [_cjk_font, 'DejaVu Sans'] if _cjk_font else ['DejaVu Sans']\n"
                    "print(f'[Sandbox] CJK font: {_cjk_font or \"NONE (Chinese may be garbled)\"}')\n"
                    "\n"
                    "# ── 标准学术配色（所有图表统一使用）────────────────────\n"
                    "COLORS = {\n"
                    "    'primary':   '#2E5B88',\n"
